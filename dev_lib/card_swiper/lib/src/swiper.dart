...
class ScaleAndFadeTransformer extends PageTransformer {
  ...
  Widget transform(Widget child, TransformInfo info) {
    ...
    if (_scale != null) {
      final scaleFactor = (1 - position!.abs()) * (1 - (_scale ?? 0.0));
      final scale = (_scale ?? 1.0) + scaleFactor;
    ...
    if (_fade != null) {
      final fadeFactor = (1 - position!.abs()) * (1 - (_fade ?? 0.0));
      final opacity = (_fade ?? 1.0) + fadeFactor;
      c = Opacity(
        opacity: opacity,
        child: c,
      );
